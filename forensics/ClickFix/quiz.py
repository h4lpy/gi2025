...
QUESTIONS = [
    {"q": "What is the FQDN of the suspected compromised system? (e.g., host1.local)", "a": "ws01.e-corp.com"},
    {"q": "What user executed the suspicious PowerShell command?", "a": "oparker"},
    {"q": "What is the full path of the registry key stores entries of execution via Win+R?", "a": "HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Explorer\RunMRU"},
    {"q": "Which URL did the user visit just prior to this event? (e.g., http://example.com)", "a": "http://verify.you-are-human.live"},
    {"q": "What is the full path of the file downloaded and executed from the PowerShell command? (e.g., http://example.com/bad.exe)", "a": "http://verify.you-are-human.live/s2.ps1"},
    {"q": "Analyse the second stage, what is the name of the third stage?", "a": "reflexiveControl.exe"},
    {"q": "Did this successfully download and execute the third stage on the host? (Yay/Nay)", "a": "Nay"},
    {"q": "Keep analysing the second stage, find the flag (format: flag{…})", "a": "flag{n3v3r_c0py_4nd_p4st3_c0d3}"}

]


def make_flag() -> str:
    return "flag{cl1ckf1x_ch4ll3ng3_c0mpl3t3}"
...
"""
        banner = B + G + ascii_art + RESET + D + " "*18 + "Fix You - [forensics]\n" + RESET + "\n"
